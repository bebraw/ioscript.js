...
function main(o, input, output) {
    process(o, input, output, function cb(err) {
        if(err) console.error(err);
    });
}

function process(o, input, output, cb) {
    fs.stat(input, function(err, stats) {
        if(!stats) return cb('Missing input!');
        if(stats.isDirectory()) processDirectory(o, input, output, cb);
        if(stats.isFile()) processFile(o, input, output, cb);
    });
}
...
function processDirectory(o, input, output, cb) {
    ...
    mkdir(output, function() {
        fs.readdir(input, function(err, files) {
            async.eachLimit(files, o.limit, function(file, cb) {
                process(o, path.join(input, file), path.join(output, file), cb);
            });
        }, cb);
    });
}
...
function write(o, input, output, data, cb) {
    ...
    // replace ext in output
    var dir = path.dirname(output);
    var base = path.basename(output, '.' + o.inputExtension);
    var target = path.join(dir, base) + '.' + o.outputExtension;
